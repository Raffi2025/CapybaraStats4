...
def XBHPercentage(arr):
    for player in arr:
        try: 
            player["XBX%"] = ((player["2B"]+player["3B"]+player["HR"])/player["H"])*100
        except:
            player["XBX%"] = 0
